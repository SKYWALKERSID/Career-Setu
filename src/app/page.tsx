    ...
    <main className="min-h-screen bg-[#f5f9fd] text-[#10295d]">
      <header className="border-b border-[#dce7f2] bg-white">
        <div className="mx-auto flex h-[68px] max-w-[1240px] items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex items-center gap-2"><button aria-label="Search" className="hidden rounded-md p-2 text-slate-500 hover:bg-slate-50 sm:block"><Search className="h-4 w-4" /></button><Link href="/login"><Button variant="outline" size="sm">Login</Button></Link><Link href="/onboarding"><Button variant="govt" size="sm">Get Started <ArrowRight className="h-4 w-4" /></Button></Link></div>
        </div>
      </header>

      <section id="home" className="border-b border-[#dce7f2] bg-[#eaf4ff]"><div className="mx-auto grid min-h-[510px] max-w-[1240px] items-center gap-10 px-5 py-14 lg:grid-cols-[1fr_0.92fr] lg:px-8 lg:py-20"><div className="max-w-[640px]"><p className="mb-5 text-[11px] font-bold uppercase tracking-[0.22em] text-[#1559c7]">Career readiness for MP students</p><h1 className="text-5xl font-extrabold leading-[1.04] tracking-[-0.03em] text-[#10295d] sm:text-6xl">Your Skills.<br /><span className="text-[#1559c7]">A Stronger Madhya Pradesh.</span></h1><p className="mt-6 max-w-[540px] text-[17px] leading-7 text-[#536987]">Personalized career guidance, skill development, and real opportunities in one place.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/onboarding"><Button variant="govt" size="lg">Get Started <ArrowRight className="h-5 w-5" /></Button></Link><Link href="#features"><Button variant="outline" size="lg">Explore the platform</Button></Link></div></div><div className="relative min-h-[310px] overflow-hidden rounded-[10px] border border-white bg-[#c8def1] shadow-[0_16px_35px_rgba(25,70,120,0.12)]"><div className="absolute inset-0 bg-[url('/landing-student-hero.jpg')] bg-cover bg-center" aria-hidden="true" /><div className="absolute inset-0 bg-gradient-to-r from-[#dceeff]/60 via-transparent to-transparent" /><div className="relative flex h-full min-h-[310px] flex-col justify-between p-7"><span className="w-fit rounded-full bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#1559c7] backdrop-blur-sm">Learn. Build. Grow.</span><p className="max-w-[210px] self-end rounded-lg bg-white/80 p-3 text-right text-lg font-bold leading-tight text-[#10295d] backdrop-blur-sm shadow-sm">Skills that open doors to a brighter future.</p></div></div></div></section>
