...
export default function OnboardingPage() {
  ...
  if (completed) {
    return <main className="grid min-h-screen bg-white lg:grid-cols-[0.9fr_1.1fr]"><section className="flex flex-col justify-center px-8 py-12 sm:px-16 lg:px-20"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#2f7d3d] text-[10px] font-bold text-white">MP</span><span><strong className="block text-[15px] text-[#10295d]">MP CareerSetu</strong><small className="text-[10px] text-slate-500">Government of Madhya Pradesh</small></span></div><div className="mt-16 max-w-[440px]"><div className="mb-7 flex gap-1">{[1, 2, 3, 4, 5].map((item) => <span key={item} className="h-1.5 flex-1 rounded-full bg-[#1559c7]" />)}<span className="ml-3 text-xs text-[#536987]">Step 5 of 5</span></div><h1 className="text-4xl font-extrabold text-[#10295d]">You&apos;re All Set!</h1><p className="mt-3 text-base leading-6 text-[#536987]">Let&apos;s build a brighter future together. Start exploring, learning and unlocking new opportunities.</p><div className="mt-8 space-y-4">{['Profile ready', 'Personalized recommendations', 'Access to courses & opportunities', 'Tools for career growth', 'Be a part of a stronger Madhya Pradesh'].map((item) => <p key={item} className="flex items-center gap-3 text-sm text-[#536987]"><CheckCircle className="h-5 w-5 shrink-0 fill-[#3f9b54] text-white" />{item}</p>)}</div><Button variant="govt" size="lg" className="mt-9 w-[215px]" onClick={() => router.push('/dashboard')}>Go to Dashboard <ArrowRight className="h-4 w-4" /></Button></div></section><section className="hidden bg-[url('/onboarding-complete-reference.jpeg')] bg-cover bg-center lg:block" aria-label="Student exploring a brighter future" /></main>;
  }
